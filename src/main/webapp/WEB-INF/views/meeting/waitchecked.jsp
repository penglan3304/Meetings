...
<script type="text/html" id="barDemo">
        ...
        <a class="layui-btn layui-btn-xs" lay-event="notify">提醒审核</a>
      </script>
<script>
    layui.use(['tree','table','layer','form','upload'], function(){
      ...
      //获取系统当前时间
       ...
       table.render({
        ...
        else if(layEvent === 'notify'){ //提醒审核
        	    layer.confirm('是否提醒审核人', function(index){
        	    	$.ajax({
                		url: "${ctx}/meeting/notify.do?ids="+data.id,
                		type: "POST",
                		dataType: "json",
               			success: function(data){            
                    		if(data==0){
                    			layer.alert('提醒成功',function(index){
                    				location.reload();
             					   layer.closeAll();
             				   });
                    			...
                    			layer.msg("已提醒，请勿重新提醒", {icon: 5}); 
                    		}
                    		else{    
                    			layer.msg("提醒失败", {icon: 5}); 
                   			 }
                		},
                		error:function(){
                			layer.msg("提醒失败", {icon: 5}); 
